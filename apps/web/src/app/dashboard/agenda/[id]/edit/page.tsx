...
export default function EditEventPage() {
  ...
  const { data: event, isLoading } = useQuery({
    queryKey: ['agenda-event', params.id],
    queryFn: () => agendaApi.get(params.id).then((r) => r.data),
  });
  ...
  const { register, handleSubmit, reset, setValue, formState: { errors, isSubmitting } } = useForm<FormData>({
    ...
    defaultValues: { allDay: false, isImportant: false, classIds: [] },
  });

  useEffect(() => {
    if (!event) return;
    const ids = event.eventClasses?.map((ec: { classId: string }) => ec.classId) ?? [];
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      await agendaApi.update(params.id, {
        ...data,
      ...
      setError(e.response?.data?.error ?? 'Erro ao salvar evento.');
    }
  }
    ...
    <div className="max-w-xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{event?.eventType ? EVENT_TYPE_LABELS[event.eventType] : ''}</p>
        </div>
      </div>
